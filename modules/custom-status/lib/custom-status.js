jQuery(document).ready(function() {
	...
	if ( $('select[name="_status"]').length == 0 ) { // not on quick edit
		...
		if ( current_user_can_publish_posts || ( current_status == 'publish' && current_user_can_edit_published_posts ) ) {
			...
			$('.save-post-status').click(function() {
				$('#post-status-display').text($('select[name="post_status"] :selected').text());
				return false;
			});
		}
	}
	...
	if ( $('select[name="post_status"]').length > 0 ) {
		...
		if ( !ef_can_change_status(current_status) ) {
			...
			var $option = $('<option></option>').text(current_status_name).attr('value', current_status).attr('selected', 'selected');
		...
		$( '#bulk-edit' ).find( 'select[name="_status"]' ).prepend( '<option value="">' + ef_text_no_change + '</option>' );
	...
	function ef_remove_post_title_trailing_dashes() {
		$('.post-title.column-title strong').each(function() {
			$(this).html($(this).html().replace(/(.*) - (<span class="post-state".*<\/span>)$/g, "$1$2"));
		});
	}
