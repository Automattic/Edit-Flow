...
class EF_Custom_Status extends EF_Module {
	...
	public function fix_post_name( $post_id, $post ) {
		...
		//Simulate how slugs are generated by WordPress in get_sample_permalink()
		if ( !get_post_meta($post->ID, 'edit_flow_change_on_permalink', true) ){
			if( empty( $_POST['post_name'] ) ) {
				$finalized_slug = sanitize_title( !empty($post->post_title) ? $post->post_title : $post->ID );
				update_post_meta( $post->ID, 'edit_flow_post_name', $finalized_slug );
			} else if ( $post->post_name != $post->post_title && $post->post_name == get_post_meta( $post->ID, 'edit_flow_post_name', true ) ) { 
				$finalized_slug = sanitize_title( !empty($post->post_title) ? $post->post_title : $post->ID );
				update_post_meta( $post->ID, 'edit_flow_post_name', $finalized_slug );
			} else {
				$finalized_slug = sanitize_title($_POST['post_name']);
				update_post_meta( $post->ID, 'edit_flow_post_name', $finalized_slug );
				update_post_meta( $post->ID, 'edit_flow_change_on_permalink', true );
			}
			$wpdb->update( $wpdb->posts, array( 'post_name' => $finalized_slug ), array( 'ID' => $post_id ) );
			clean_post_cache( $post_id );
		} else {
			$finalized_slug = sanitize_title( $_POST['post_name'] );
			update_post_meta( $post->ID, 'edit_flow_post_name', $finalized_slug );
