...
class EF_Settings extends EF_Module {
	...
	function print_default_header( $current_module ) {
		...
		<div class="wrap edit-flow-admin">
			...
			<h2><a href="<?php echo esc_url(EDIT_FLOW_SETTINGS_PAGE); ?>"><?php _e('Edit Flow', 'edit-flow') ?></a>:&nbsp;<?php echo esc_attr($current_module->title); ?><?php if ( isset( $display_text ) ) { echo wp_kses($display_text, 'a'); } ?></h2>
			<?php else: ?>
			<?php echo wp_kses_post($page_icon); ?>
			<h2><?php _e('Edit Flow', 'edit-flow') ?><?php if ( isset( $display_text ) ) { echo wp_kses($display_text, 'a'); } ?></h2>
			<?php endif; ?>
	...
	function settings_vip_features_option() {
		$options = array(
	...
	function settings_analytics_option() {
		$options = array(
			'off' => __( 'Disabled', 'edit-flow' ),
			'on' => __( 'Enabled', 'edit-flow' ),
		);
