...
( function ( $ ) {
	...
	$.extend( Timepicker.prototype, {
		...
		_parseTime( timeString, withDate ) {
			...
			if ( withDate || ! this._defaults.timeOnly ) {
				const dp_dateFormat = $.datepicker._get( this.inst, 'dateFormat' );
				try {
					const parseRes = parseDateTimeInternal(
						dp_dateFormat,
			...
			const timeObj = $.datepicker.parseTime(
				this._defaults.timeFormat,
				timeString,
				this._defaults
			);
		...
		_injectTimePicker() {
			...
			if ( $dp.find( 'div.ui-timepicker-div' ).length === 0 && o.showTimepicker ) {
					...
					if ( o[ 'show' + uitem ] && o[ litem + 'Grid' ] > 0 ) {
						...
						if ( litem == 'hour' ) {
							for (
								...
								gridSize[ litem ]++;
								const tmph = $.datepicker.formatTime(
					...
					if ( o[ 'show' + uitem ] && o[ litem + 'Grid' ] > 0 ) {
						...
						$tp
							...
							.on( 'click', function ( e ) {
								const $t = $( this );
								const h = $t.html();
								let n = parseInt( h.replace( /[^0-9]/g ), 10 );
								const ap = h.replace( /[^apm]/gi );
								const f = $t.data( 'for' ); // loses scope, so we use data-for
				...
				if (
					...
					const local_timezone = $.timepicker.timeZoneOffsetString( local_date );
				...
				const $buttonPanel = $dp.find( '.ui-datepicker-buttonpane' );
				...
				if ( this.inst !== null ) {
					const timeDefined = this.timeDefined;
					this._onTimeChange();
				...
				if ( this._defaults.addSliderAccess ) {
					const sliderAccessArgs = this._defaults.sliderAccessArgs;
					const rtl = this._defaults.isRTL;
					...
					setTimeout( function () {
						...
						if ( $tp.find( '.ui-slider-access' ).length === 0 ) {
							...
							const sliderAccessWidth = $tp.find( '.ui-slider-access:eq(0)' ).outerWidth( true );
							if ( sliderAccessWidth ) {
								$tp.find( 'table:visible' ).each( function () {
									const $g = $( this );
									const oldWidth = $g.outerWidth();
									const oldMarginLeft = $g
										.css( rtl ? 'marginRight' : 'marginLeft' )
										.toString()
										.replace( '%', '' );
									const newWidth = oldWidth - sliderAccessWidth;
									const newMarginLeft = ( oldMarginLeft * newWidth ) / oldWidth + '%';
									const css = { width: newWidth, marginRight: 0, marginLeft: 0 };
		...
		_limitMinMaxDateTime( dp_inst, adjustSliders ) {
			...
			const dp_date = new Date( dp_inst.selectedYear, dp_inst.selectedMonth, dp_inst.selectedDay );
			...
			if (
				...
				const minDateTime = $.datepicker._get( dp_inst, 'minDateTime' );
				const minDateTimeDate = new Date(
			...
			if (
				...
				const maxDateTime = $.datepicker._get( dp_inst, 'maxDateTime' );
				const maxDateTimeDate = new Date(
			...
			if ( adjustSliders !== undefined && adjustSliders === true ) {
				const hourMax = parseInt(
					this._defaults.hourMax -
						( ( this._defaults.hourMax - this._defaults.hourMin ) % this._defaults.stepHour ),
					10
				);
				const minMax = parseInt(
					this._defaults.minuteMax -
						( ( this._defaults.minuteMax - this._defaults.minuteMin ) % this._defaults.stepMinute ),
					10
				);
				const secMax = parseInt(
					this._defaults.secondMax -
						( ( this._defaults.secondMax - this._defaults.secondMin ) % this._defaults.stepSecond ),
					10
				);
				const millisecMax = parseInt(
		 ...
		 */
		_onTimeChange() {
			let hour = this.hour_slider ? this.control.value( this, this.hour_slider, 'hour' ) : false;
			let minute = this.minute_slider
				? this.control.value( this, this.minute_slider, 'minute' )
				: false;
			let second = this.second_slider
				? this.control.value( this, this.second_slider, 'second' )
				: false;
			let millisec = this.millisec_slider
				? this.control.value( this, this.millisec_slider, 'millisec' )
				: false;
			let timezone = this.timezone_select ? this.timezone_select.val() : false;
			const o = this._defaults;
			const pickerTimeFormat = o.pickerTimeFormat || o.timeFormat;
			const pickerTimeSuffix = o.pickerTimeSuffix || o.timeSuffix;
			...
			const ampm = o[ hour < 12 ? 'amNames' : 'pmNames' ][ 0 ];
			...
			if ( this.$timeObj ) {
				if ( pickerTimeFormat === o.timeFormat ) {
					...
					this.$timeObj.text(
						$.datepicker.formatTime( pickerTimeFormat, this, o ) + pickerTimeSuffix
					);
				}
			}
		 ...
		 */
		_onSelectHandler() {
			const onSelect = this._defaults.onSelect || this.inst.settings.onSelect;
			const inputEl = this.$input ? this.$input[ 0 ] : null;
			if ( onSelect && inputEl ) {
		...
		_updateDateTime( dp_inst ) {
			...
			const dt = $.datepicker._daylightSavingAdjust(
				new Date( dp_inst.selectedYear, dp_inst.selectedMonth, dp_inst.selectedDay )
			);
			const dateFmt = $.datepicker._get( dp_inst, 'dateFormat' );
			const formatCfg = $.datepicker._getFormatConfig( dp_inst );
			const timeAvailable = dt !== null && this.timeDefined;
			...
			if ( this._defaults.timeOnly === true ) {
				...
				formattedDateTime +=
					this._defaults.separator + this.formattedTime + this._defaults.timeSuffix;
			}
			...
			if ( ! this._defaults.showTimepicker ) {
				...
				let altFormattedDateTime = '';
				const altSeparator = this._defaults.altSeparator
					? this._defaults.altSeparator
					: this._defaults.separator;
				const altTimeSuffix = this._defaults.altTimeSuffix
		...
		_onFocus() {
			if ( ! this.$input.val() && this._defaults.defaultValue ) {
				...
				const inst = $.datepicker._getInst( this.$input.get( 0 ) );
			...
			select: {
				create( tp_inst, obj, unit, val, min, max, step ) {
					let sel =
						...
						'">';
					const ul =
						tp_inst._defaults.timeFormat.indexOf( 't' ) !== -1 ? 'toLowerCase' : 'toUpperCase';
					const m = 0;
					...
					for ( let i = min; i <= max; i += step ) {
						...
						if (
							unit == 'hour' &&
							useAmpm( tp_inst._defaults.pickerTimeFormat || tp_inst._defaults.timeFormat )
						) {
	...
	$.fn.extend( {
		...
		timepicker( o ) {
			o = o || {};
			const tmp_args = Array.prototype.slice.call( arguments );
	...
	$.datepicker.parseTime = function ( timeFormat, timeString, options ) {
		const o = extendRemove( extendRemove( {}, $.timepicker._defaults ), options || {} );

		// Strict parse requires the timeString to match the timeFormat exactly
		const strictParse = function ( f, s, o ) {
			// pattern for standard and localized AM/PM markers
			const getPatternAmpm = function ( amNames, pmNames ) {
				let markers = [];
				if ( amNames ) {
			...
			const getFormatPositions = function ( timeFormat ) {
				const finds = timeFormat
					.toLowerCase()
			...
			const regstr =
				...
				o.timeSuffix +
				'$';
			const order = getFormatPositions( f );
			let ampm = '';
			let treg;
			...
			if ( treg ) {
				...
				if ( order.z !== -1 && treg[ order.z ] !== undefined ) {
					let tz = treg[ order.z ].toUpperCase();
					switch ( tz.length ) {
						case 1:
							// Z
		...
		const looseParse = function ( f, s, o ) {
			try {
				let d = new Date( '2012-01-01 ' + s );
				if ( isNaN( d.getTime() ) ) {
	...
	$.datepicker.formatTime = function ( format, time, options ) {
		...
		let tmptime = format;
		let ampmName = options.amNames[ 0 ];
	...
	*/
	...
	$.datepicker._selectDate = function ( id, dateStr ) {
		...
		const tp_inst = this._get( inst, 'timepicker' );
	...
	$.datepicker._doKeyPress = function ( event ) {
		...
		if ( tp_inst ) {
			if ( $.datepicker._get( inst, 'constrainInput' ) ) {
				const ampm = useAmpm( tp_inst._defaults.timeFormat );
				const dateChars = $.datepicker._possibleChars( $.datepicker._get( inst, 'dateFormat' ) );
				const datetimeChars =
	...
	$.datepicker._updateAlternate = function ( inst ) {
		const tp_inst = this._get( inst, 'timepicker' );
		if ( tp_inst ) {
			const altField = tp_inst._defaults.altField;
			if ( altField ) {
				// update alternate field too
				const altFormat = tp_inst._defaults.altFormat || tp_inst._defaults.dateFormat;
				const date = this._getDate( inst );
				const formatCfg = $.datepicker._getFormatConfig( inst );
				let altFormattedDateTime = '';
				const altSeparator = tp_inst._defaults.altSeparator
					? tp_inst._defaults.altSeparator
					: tp_inst._defaults.separator;
				const altTimeSuffix = tp_inst._defaults.altTimeSuffix
				...
				altFormattedDateTime +=
					$.datepicker.formatTime( altTimeFormat, tp_inst, tp_inst._defaults ) + altTimeSuffix;
				if (
	...
	$.datepicker._setTime = function ( inst, date ) {
		const tp_inst = this._get( inst, 'timepicker' );
		if ( tp_inst ) {
			const defaults = tp_inst._defaults;
	...
	$.datepicker._setDateDatepicker = function ( target, date ) {
		...
		const tp_date = date instanceof Date ? new Date( date.getTime() ) : date;
	...
	$.datepicker.parseDate = function ( format, value, settings ) {
		...
		try {
			...
			date = this._base_parseDate(
				format,
				value.substring( 0, value.length - ( err.length - err.indexOf( ':' ) - 2 ) ),
				settings
			);
			$.timepicker.log(
	...
	$.datepicker._optionDatepicker = function ( target, name, value ) {
		...
		if ( tp_inst ) {
			...
			let max = null;
			let onselect = null;
			const overrides = tp_inst._defaults.evnts;
			const fns = {};
			let prop;
	 ...
	 */
	const splitDateTime = function ( dateFormat, dateTimeString, dateSettings, timeSettings ) {
		try {
			// The idea is to get the number separator occurances in datetime and the time format requested (since time has
			// fewer unknowns, mostly numbers and am/pm). We will use the time pattern to split.
			const separator =
				timeSettings && timeSettings.separator
					? timeSettings.separator
					: $.timepicker._defaults.separator;
			const format =
				timeSettings && timeSettings.timeFormat
					? timeSettings.timeFormat
					: $.timepicker._defaults.timeFormat;
			const timeParts = format.split( separator ); // how many occurances of separator may be in our format?
			const timePartsLen = timeParts.length;
			const allParts = dateTimeString.split( separator );
			const allPartsLen = allParts.length;
			...
			if ( err.indexOf( ':' ) >= 0 ) {
				...
				// the "extra" characters.  We rely on that instead of
				// attempting to perfectly reproduce the parsing algorithm.
				const dateStringLength = dateTimeString.length - ( err.length - err.indexOf( ':' ) - 2 );
				const timeString = dateTimeString.substring( dateStringLength );

				return [
					$.trim( dateTimeString.substring( 0, dateStringLength ) ),
	...
	var parseDateTimeInternal = function (
		...
		let date;
		const splitRes = splitDateTime( dateFormat, dateTimeString, dateSettings, timeSettings );
		date = $.datepicker._base_parseDate( dateFormat, splitRes[ 0 ], dateSettings );
		if ( splitRes[ 1 ] !== '' ) {
			const timeString = splitRes[ 1 ];
			const parsedTime = $.datepicker.parseTime( timeFormat, timeString, timeSettings );
	...
	var selectLocalTimeZone = function ( tp_inst, date ) {
		if ( tp_inst && tp_inst.timezone_select ) {
			...
			const now = typeof date !== 'undefined' ? date : new Date();
			let tzoffset = $.timepicker.timeZoneOffsetString( now );
			if ( tp_inst._defaults.timezoneIso8601 ) {
	...
	$.timepicker.handleRange = function ( method, startTime, endTime, options ) {
		...
		if ( method != 'timepicker' && options.reformat ) {
			$( [ startTime, endTime ] ).each( function () {
				const format = $( this )[ method ].call( $( this ), 'option', 'dateFormat' );
		...
		function selected( changed, other, option ) {
			...
			const date = $( changed )[ method ].call( $( changed ), 'getDate' );
