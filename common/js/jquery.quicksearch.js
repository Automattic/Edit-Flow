jQuery( function ( $ ) {
	$.fn.quicksearch = function ( opt ) {
		...
		function get_key() {
			const input = strip_html( $( 'input[rel="' + options.randomElement + '"]' ).val() );
		...
		function stripe( el ) {
			if ( doStripe ) {
				...
				select_element( el )
					.filter( ':visible' )
					.each( function () {
						for ( let j = 0; j < stripeRowLength; j++ ) {
							if ( i === j ) {
		...
		function loader( o ) {
			if ( options.loaderId ) {
				const l = $( 'input[rel="' + options.randomElement + '"]' )
					.parent()
					.find( '.loader' );
				if ( o === 'hide' ) {
					l.hide();
		...
		function place_form() {
			const formPosition = options.position;
			const formAttached = options.attached;
		...
		function make_form_input() {
			const val = ! is_empty( options.inputText ) ? options.inputText : '';
			return (
		...
		function make_form() {
			const f = ! options.isFieldset ? 'form' : 'fieldset';
			return (
				'<' +
				f +
				' action="#" ' +
		...
		$( 'form.quicksearch, fieldset.quicksearch' ).on( 'submit', function () {
			return false;
		} );
